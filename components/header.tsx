  ...
  return (
    ...
    <header className="z-[999] relative">
      ...
      <nav className="flex fixed top-[0.15rem] left-1/2 h-12 -translate-x-1/2 py-2 sm:h-[initial] sm:py-0">
        <ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-400 sm:w-[initial] sm:flex-nowrap sm:gap-16">
          {links.map((link) => (
            <motion.li
            ...
            >
              <Link
                className="flex w-full items-center justify-center px-3 py-3 transition text-gray-400 hover:text-gray-300"
                href={link.hash}
              >
                {link.name}
              </Link>
